    ...
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        ...
        if (diffDays === 0) {
          ...
          indicatorIcon = '<i class="bi bi-exclamation-circle-fill text-danger"></i>'; // Red indicator
        } else if (diffDays === 1) {
          dueDateText = "due tomorrow";
          indicatorIcon = '<i class="bi bi-exclamation-circle-fill text-warning"></i>'; // Yellow indicator
        } else if (diffDays > 1) {
          dueDateText = "due in " + diffDays + " days";
          indicatorIcon = '<i class="bi bi-check-circle-fill text-success"></i>'; // Green indicator
        } else {
          dueDateText = "Overdue " + diffDays + " days";
          indicatorIcon = '<i class="bi bi-exclamation-diamond-fill text-danger"></i>'; // Red indicator for overdue
        }

        // Append each todo to the todoList table with indicator icon
        todoList.innerHTML += `
          <tr>

              <td>${indicatorIcon}${todoData.taskType}</td>
              <td>${dueDateText}</td>
              <td>${todoData.cost}</td>
...
document.addEventListener("DOMContentLoaded", function () {
 ...
 // Function to update the real-time date and time
    ...
    // Update the real-time date and time every second
